...
| [fortinet_fortimanager](plugins/fortinet_fortimanager/) | 3.1.1 | Fix Create Address Object failing on an existing object (FortiManager returns code -2, not -6); add Get Address Group and List Address Groups actions; normalize FortiManager address responses to fix action crashes, remove Enable Search so Check if Address in Group always matches name and value, add Message output to all five address actions |
